...
export const manualProjects: ManualProject[] = [
  {
    ...
    detail:
      'My capstone. Most medical chatbots answer confidently and cite nothing, which is the exact failure mode that makes them unusable in practice. MediQuery retrieves with dense embeddings and BM25 in parallel, fuses the rankings with Reciprocal Rank Fusion, and attaches an explainability score built from retrieval confidence, generation confidence, and source attribution - so a wrong answer is visibly a low-confidence answer. Backed by a 16-file pytest suite and a 6-stage CI/CD pipeline covering linting, testing, and security scanning.',
  ...
  },
  {
    ...
    detail:
      'Managing firmware on deployed IoT hardware is the part nobody demos. This handles LED and relay control, RS485 solar meter reads, and MQTT-driven OTA updates across a device fleet - with flashing that runs in the browser via WebSerial, so a field technician needs no toolchain. Firmware artifacts live in Cloudflare R2.',
  ...
  },
  {
    ...
    blurb: 'A custom CNN that reads handwritten maths and solves it - 14 symbols at 95% test accuracy.',
    detail:
      'An OpenCV pipeline normalizes input to 32x32 grayscale and segments individual symbols, then a custom CNN classifies 14 mathematical symbols at 95% test accuracy before the expression is parsed and evaluated. Next.js frontend on Vercel, FastAPI inference API on Hugging Face Spaces.',
  ...
  },
  {
    ...
    detail:
      'Payment aggregators take a cut and require a merchant account. This skips both: payments go straight to the bank account, and an n8n workflow watches the bank\'s credit-alert emails, matches them against pending orders, and fires a webhook back into Django to confirm. A pragmatic solution to a real constraint for small Indian merchants.',
  ...
  },
  {
    ...
    detail:
      'The desktop counterpart to the ESP32 fleet dashboard. Native Rust/egui application that pushes firmware to devices over MQTT with TLS, pulling artifacts from Cloudflare R2.',
  ...
  },
  {
    ...
    blurb: 'End-to-end ML pipeline on the Kaggle house prices set - supervised regression, clustering, and SHAP interpretability.',
    detail:
  ...
  },
  {
    ...
    liveNote: 'Hosted on Streamlit Cloud - may take ~30s to wake.',
  },
  {
    slug: 'ensemble-heart-disease-prediction',
    title: 'Heart Disease Ensemble',
    blurb: 'Earlier ensemble approach to heart disease prediction, deployed on Streamlit.',
  ...
  },
  {
    ...
    detail:
      'Curated Java coding solutions and MCQ notes for the TCS Xplore IPA/NQT assessment, alongside a self-built interactive practice web app.',
